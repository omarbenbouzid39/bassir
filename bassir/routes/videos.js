...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowed = ['video/mp4','video/quicktime','video/webm','video/avi','image/jpeg','image/png'];
...
router.get('/', authOptional, async (req, res) => {
  try {
    ...
    res.json({ videos, total: total?.n || 0, page: Number(page) });
  } catch (err) {
...
router.post('/', authRequired, upload.single('video'), async (req, res) => {
  try {
    ...
    const video = await queryOne(
      'INSERT INTO videos (user_id,title,description,category,tags,visibility,filename) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id,title,filename',
...
router.delete('/:id', authRequired, async (req, res) => {
  try {
    const video = await queryOne('SELECT * FROM videos WHERE id=$1 AND user_id=$2', [req.params.id, req.user.id]);
...
router.post('/:id/like', authRequired, async (req, res) => {
  try {
    const existing = await queryOne('SELECT id FROM likes WHERE user_id=$1 AND video_id=$2', [req.user.id, req.params.id]);
    if (existing) {
...
router.post('/:id/save', authRequired, async (req, res) => {
  try {
    const existing = await queryOne('SELECT id FROM saves WHERE user_id=$1 AND video_id=$2', [req.user.id, req.params.id]);
    if (existing) {
