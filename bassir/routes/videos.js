...
router.get('/', authOptional, async (req, res) => {
  try {
    ...
    // ✅ إصلاح visibility:followers — يظهر للمتابعين ولصاحب الفيديو
    let visFilter;
    if (userId) {
      const followingIds = await Follow.find({ follower_id: userId }).distinct('following_id');
      visFilter = { $or: [
        { visibility: 'public' },
        { user_id: userId },
    ...
    const [likes, saves, comments, userLikes, userSaves] = await Promise.all([
      Like.aggregate([{ $match: { video_id: { $in: videoIds } } }, { $group: { _id: '$video_id', count: { $sum: 1 } } }]),
      Save.aggregate([{ $match: { video_id: { $in: videoIds } } }, { $group: { _id: '$video_id', count: { $sum: 1 } } }]),
    ...
    const savesMap    = Object.fromEntries(saves.map(s => [s._id.toString(), s.count]));
    ...
    const result = videos.map(v => ({
      ...
      saves_count:    savesMap[v._id.toString()]    || 0,
  ...
  try {
    ...
    const result = videos
      .map(v => ({
        ...
        author_handle: v.user_id?.handle,
        likes_count:   likesMap[v._id.toString()] || 0,
        score:         v.views + (likesMap[v._id.toString()] || 0) * 10,
      }))
...
// ── POST /api/videos/:id/view — يُستدعى من الـ feed عند مشاهدة الفيديو ────────
router.post('/:id/view', async (req, res) => {
  try {
    if (!mongoose.isValidObjectId(req.params.id))
      return res.status(200).json({ ok: true }); // نتجاهل بصمت
    await Video.findByIdAndUpdate(req.params.id, { $inc: { views: 1 } });
    res.json({ ok: true });
  } catch {
    res.json({ ok: true }); // لا نريد إيقاف الـ UI بسبب عدّ المشاهدات
  }
});
